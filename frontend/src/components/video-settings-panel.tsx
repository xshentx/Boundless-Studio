...
import { isSeedanceFastModel, isSeedanceVideoConfig, normalizeSeedanceDuration, normalizeSeedanceRatio, normalizeSeedanceResolution, seedancePixelLabel, seedanceRatioOptions, seedanceResolutionOptions } from "@/lib/seedance-video";
...
const SEEDANCE_VISIBLE_RESOLUTION_OPTIONS = seedanceResolutionOptions.filter((item) => item.value !== "480p");
const SEEDANCE_VISIBLE_RATIO_OPTIONS = seedanceRatioOptions.filter((item) => item.value !== "adaptive");
const SEEDANCE_VISIBLE_DURATION_OPTIONS = [5, 10, 15] as const;
...
function SeedanceVideoSettingsPanel({ config, onConfigChange, theme, showTitle, className }: VideoSettingsPanelProps) {
    ...
    const resolution = SEEDANCE_VISIBLE_RESOLUTION_OPTIONS.some((item) => item.value === normalizedResolution) ? normalizedResolution : "720p";
    const normalizedRatio = normalizeSeedanceRatio(config.size);
    const ratio = SEEDANCE_VISIBLE_RATIO_OPTIONS.some((item) => item.value === normalizedRatio) ? normalizedRatio : "16:9";
    const normalizedDuration = normalizeSeedanceDuration(config.videoSeconds);
    const duration = SEEDANCE_VISIBLE_DURATION_OPTIONS.some((value) => value === normalizedDuration) ? normalizedDuration : 5;
        ...
        <ImageSettingsTheme theme={theme}>
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                <SettingGroup title="分辨率" color={theme.node.muted}>
                    <div className="grid grid-cols-3 gap-2.5">
                        {SEEDANCE_VISIBLE_RESOLUTION_OPTIONS.map((item) => {
                ...
                <SettingGroup title="比例" color={theme.node.muted}>
                    <div className="grid grid-cols-3 gap-2.5">
                        {SEEDANCE_VISIBLE_RATIO_OPTIONS.map((item) => (
                            <button
                                key={item.value}
                                type="button"
                ...
                <SettingGroup title="时长" color={theme.node.muted}>
                    <div className="grid grid-cols-4 gap-2.5">
                        {SEEDANCE_VISIBLE_DURATION_OPTIONS.map((value) => (
...
export function videoSecondsLabel(value: string) {
    ...
    const visibleDuration = SEEDANCE_VISIBLE_DURATION_OPTIONS.some((item) => item === duration) ? duration : 5;
    return `${visibleDuration}s`;
}
