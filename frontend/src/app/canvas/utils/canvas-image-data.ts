...
export async function splitDataUrl(dataUrl: string, params: ImageSplitParams): Promise<ImageSplitPiece[]> {
    ...
    const rowBounds = splitBounds(image.height, rows, params.rowCuts);
    const columnBounds = splitBounds(image.width, columns, params.columnCuts);
    const pieces: ImageSplitPiece[] = [];

    for (let row = 0; row < rows; row += 1) {
        const sy = Math.round(rowBounds[row]);
        const sh = Math.max(1, Math.round(rowBounds[row + 1]) - sy);
        for (let column = 0; column < columns; column += 1) {
            const sx = Math.round(columnBounds[column]);
            const sw = Math.max(1, Math.round(columnBounds[column + 1]) - sx);
            pieces.push({
                row,
                column,
                x: sx,
                ...
                dataUrl: drawCrop(image, sx, sy, sw, sh),
            });
        }
    }
...
function splitBounds(size: number, count: number, cuts?: number[]) {
    const innerCuts = normalizeCuts(size, count, cuts);
    if (innerCuts.length === count - 1) return [0, ...innerCuts, size];
...
function normalizeCuts(size: number, count: number, cuts?: number[]) {
    if (!Array.isArray(cuts) || count <= 1) return [];
    const maxCuts = count - 1;
    const sorted = cuts
        .map((cut) => Math.round(Number(cut)))
        .filter((cut) => Number.isFinite(cut))
        .sort((a, b) => a - b);
    const result: number[] = [];
    for (const cut of sorted) {
        const min = (result[result.length - 1] || 0) + 1;
        const max = size - (maxCuts - result.length);
        result.push(Math.min(max, Math.max(min, cut)));
