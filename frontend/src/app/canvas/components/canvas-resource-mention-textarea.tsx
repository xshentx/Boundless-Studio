...
const isImeComposing = (event: React.KeyboardEvent<HTMLTextAreaElement>) => event.nativeEvent.isComposing || event.keyCode === 229;
...
export const CanvasResourceMentionTextarea = forwardRef<HTMLTextAreaElement, Props>(function CanvasResourceMentionTextarea({ value, references, onChange, onSubmit, onKeyDown, className, containerClassName, containerStyle, style, highlightLabels = true, ...props }, forwardedRef) {
    ...
    return (
        <div className={`relative h-full w-full ${containerClassName || ""}`} style={containerStyle}>
            {showOverlay ? (
            ...
            <textarea
                ...
                onKeyDown={(event) => {
                    ...
                    if (event.key === "Enter" && onSubmit && !event.ctrlKey && !event.metaKey && !event.shiftKey && !isImeComposing(event)) {
                        event.preventDefault();
                        onSubmit();
                        return;
                    }
