...
import { ImageSettingsPanel, imageQualityLabel, imageSizeLabel, type ImageSettingsSection } from "@/components/image-settings-panel";
...
export function CanvasImageSettingsPopover({ config, onConfigChange, onOpenChange, buttonClassName, placement = "topLeft", trigger, sections, showTitle }: CanvasImageSettingsPopoverProps) {
    ...
    const handleConfigChange = (key: keyof AiConfig, value: string) => {
        ...
        if (key === "count") {
            updateGlobalConfig("canvasImageCount", value);
            return;
        }
    ...
    const panel = open && buttonRect ? <ImageSettingsPortal buttonRect={buttonRect} panelRef={panelRef} placement={placement} theme={theme} config={config} sections={sections} showTitle={showTitle} onConfigChange={handleConfigChange} /> : null;
...
function ImageSettingsPortal({
    ...
    return createPortal(
        <>
            ...
            >
                <ImageSettingsPanel config={config} onConfigChange={(key, value) => onConfigChange(key, value)} theme={theme} showTitle={showTitle} className="space-y-4" maxCount={CANVAS_IMAGE_MAX_COUNT} sections={sections} />
            </div>
        </>,
        document.body,
    );
