...
| macOS Universal | `BoundlessStudio-macos-universal.zip` | macOS 10.13+，Intel 或 Apple Silicon |
...
1. 解压 `BoundlessStudio-macos-universal.zip`。
...
> Windows 与 macOS 包均由同一 `v1.0.0` 源码标签构建。Windows 应用内自动更新目前仅处理 Windows 客户端资源；macOS 请从 Releases 手动更新。
...
# macOS Universal（需在 macOS 上运行）
wails build -platform darwin/universal -s
```
...
2. 构建 Windows x64 与 macOS Universal 程序。
