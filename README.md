...
> Windows 与 macOS 包均由同一 `v1.0.7` 源码标签构建。macOS 包面向 Apple Silicon（ARM64）。Windows 应用内自动更新目前仅处理 Windows 客户端资源；macOS 请从 Releases 手动更新。
...
当前版本：**v1.0.7**。详见 [RELEASE_NOTES.md](RELEASE_NOTES.md)。